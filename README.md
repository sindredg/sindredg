...
| --- | --- | --- |
| [**Azure–GCP Cross-Cloud Integration**](https://github.com/sindredg/cross-cloud-integration) | **In progress:** Connects an Azure-hosted AD DS forest to GCP workloads over redundant BGP VPN with hybrid DNS, enabling cross-cloud domain join. Entra ID identities reach GCP through workforce identity federation, and GCP machines are managed from Azure through Arc. Azure stays the identity and management home, GCP carries the workloads. Built with Terraform against a protected remote state backend. | Terraform · Azure · GCP · Active Directory · Blob Storage backend · Entra ID RBAC · BGP/VPN · Azure Arc · Workforce Identity Federation |
