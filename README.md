...
| --- | --- | --- |
| [**From Forest to Cloud: A Two-Region Hybrid Identity Build**](https://github.com/sindredg/two-site-hybrid-identity) | One Active Directory forest in Azure spanning two regions over VNet peering, synchronised to Entra ID, with both client endpoints hybrid-joined to Active Directory and the cloud at the same time. Group Policy is linked against the OU structure and verified from the clients. Azure is declared in Terraform, the directory in idempotent PowerShell, and decisions, risks and build failures are documented alongside the build. Runs entirely on Entra ID Free. Security baselines and Windows LAPS next. | Terraform · Azure · Active Directory · Windows Server · Group Policy · PowerShell · Entra ID · Entra Connect |
