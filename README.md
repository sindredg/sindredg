...
| --- | --- | --- |
| [**Azure Hybrid Network: Building It Layer by Layer**](https://github.com/sindredg/hybrid-network-az) | Hub-and-spoke network in Azure with a simulated on-premises datacenter in another region, connected over an encrypted IPsec tunnel, built one mechanism at a time. Covers gateway transit, subnet NSGs, private access through Bastion, keyless OIDC deployment with remote state, Azure firewall with forced routing. private endpoints and cross-premises DNS. | Terraform · Azure Networking · VPN Gateway · Hub-and-Spoke · CI/CD · GitHub Actions · OIDC · Azure RBAC |
