...
| --- | --- | --- |
| [**Azure–GCP Cross-Cloud Integration**](https://github.com/sindredg/cross-cloud-integration) | **In progress.** Connects an Azure-hosted Active Directory estate to GCP workloads over redundant BGP VPN with hybrid DNS, extending identity across both clouds through Azure Arc, workforce federation and cross-cloud domain join. | Terraform · Azure · GCP · Active Directory · Blob Storage backend · Entra ID RBAC · BGP/VPN · Azure Arc · Workforce Identity Federation |
| [**Forest to Cloud: Cross-Region Hybrid Identity & Infrastructure**](https://github.com/sindredg/two-site-hybrid-identity) | A two-site Active Directory forest in Azure, synchronised to Entra ID, with hybrid-joined endpoints hardened by Group Policy and Microsoft security baselines, per-machine LAPS passwords in both AD and Entra ID, and tiered administration enforced by deny-logon policy. Onfrastructure declared in Terraform. | Terraform · Azure · Entra ID · Active Directory · Windows Server · Group Policy · Windows LAPS · PowerShell · Entra Connect Sync |
