...
| --- | --- | --- |
| [**Hybrid Identity: Active Directory to Entra ID**](https://github.com/sindredg/hybrid-identity-entraid-and-ad) | Stands up an on-premises-style Active Directory forest in Azure and prepares it for synchronisation to Entra ID: Terraform for the infrastructure, idempotent PowerShell scripts for the directory, and Bastion-only access with no internet-facing surface. Seeded with OU-scoped users and groups so sync filtering is demonstrable rather than assumed. | Terraform · Azure · Active Directory · Windows Server · PowerShell ·  Entra ID · Entra Connect · |
...
| [**Azure Infrastructure as Code with Terraform**](https://github.com/sindredg/terraform-grafana-iaas-lab) | Rebuilds a hand-built Azure environment as modular Terraform: virtual network, security rules, public IP with a stable FQDN, and a cloud-init provisioned Linux VM. Naming, tagging, and NSG rules are generated from single definitions rather than repeated per resource. | Terraform · Azure · azurerm provider · cloud-init · Docker · Azure CLI |
