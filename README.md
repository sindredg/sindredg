...
| --- | --- | --- |
| [**Azure Hybrid Network: Building It Layer by Layer**](https://github.com/sindredg/hybrid-network-az) | Hub-and-spoke network in Azure with a simulated on-premises datacenter in another region, connected over an encrypted IPsec tunnel, built one mechanism at a time. Covers gateway transit, subnet NSGs against Azure's permissive defaults, private access through Bastion, keyless OIDC deployment with remote state, and firewall with forced routing. private endpoints and cross-premises DNS are next. | Terraform · Azure Networking · VPN Gateway · Azure Firewall · CI/CD · GitHub Actions · OIDC · Azure RBAC |
