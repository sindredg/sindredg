...
| --- | --- | --- |
| [**Azure–GCP Cross-Cloud Integration**](https://github.com/sindredg/cross-cloud-integration) | **In progress.** Azure Terraform ownership and a protected remote state backend are applied and validated: Entra-only authorization, Shared Key disabled, versioning, soft delete and state locking proven against live Azure. Designed and not yet built: redundant BGP VPN and hybrid DNS to GCP, cross-cloud domain join, Entra ID federation into GCP, and Arc-managed GCP machines. Azure is the identity and management home, GCP carries the workloads. | Terraform · Azure · GCP · Active Directory · Blob Storage backend · Entra ID RBAC · BGP/VPN · Azure Arc · Workforce Identity Federation |
