...
| --- | --- | --- |
| [**Hybrid Network: Hub-Spoke, VPN and Traffic Inspection**](https://github.com/sindredg/hybrid-network-az) | Hub-and-spoke network with a simulated on-premises datacenter, joined by an IPsec tunnel. Features gateway transit so the spoke reaches on-prem through the hub's gateway, subnet NSGs verified rule by rule against Azure's permissive defaults, private VM access through Bastion, and keyless OIDC deployment with remote state. Azure Firewall with forced routing, private endpoints and cross-premises DNS resolution are planned. | Terraform · Azure Networking · VPN Gateway · Azure Firewall · CI/CD · GitHub Actions · OIDC · Azure RBAC |
