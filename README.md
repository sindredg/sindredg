...
| --- | --- | --- |
| [**Azure–GCP Cross-Cloud Integration**](https://github.com/sindredg/cross-cloud-integration) | **In progress.** Connects an Azure-hosted Active Directory estate to GCP workloads over redundant BGP VPN with hybrid DNS, extending identity across both clouds through Azure Arc, workforce federation and cross-cloud domain join. Validated so far: Terraform ownership of the live Azure estate, a protected remote state backend with Shared Key disabled and Entra-only data-plane access, and identity state migrated under no-op plan gates. | Terraform · Azure · GCP · Active Directory · Blob Storage backend · Entra ID RBAC · BGP/VPN · Azure Arc · Workforce Identity Federation |
