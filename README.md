...
| --- | --- | --- |
| [**Hybrid Identity Platform: Two Regions, One Forest, Cloud-Joined Endpoints**](https://github.com/sindredg/two-site-hybrid-identity) | An on-premises-style Active Directory estate in Azure, split across two regions and joined by VNet peering, synchronised to Entra ID with endpoints hybrid-joined to both directories. Every resource is declared in Terraform, and every design decision, known risk and build failure is documented alongside it. Runs entirely on Entra ID Free. Group Policy, security baselines and Windows LAPS next. | Terraform · Azure · VNet Peering · Active Directory · Windows Server · PowerShell · Entra ID · Entra Connect |
